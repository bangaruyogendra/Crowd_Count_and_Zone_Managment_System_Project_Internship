...
const UserManagement = () => {
  ...
  const handleDelete = async (userId) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/userDetails/${userId}/`);
      setUserDetails((prev) => prev.filter((user) => user.id !== userId));
    } catch (err) {
  ...
  return (
    <div className="dashboard">
      <div className="title-heading">
        <h1 className="title">
          People Counting and Zone Management System
        </h1>
      </div>
      <div>
